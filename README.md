# گزارش پیاده‌سازی آزمایش ۲ - درس آزمایشگاه مهندسی نرم‌افزار
...
## گام چهارم - ارزیابی تأثیر طراحی

- **افزودن پیامک:** در نسخه‌ اولیه مجبور بودیم MessageSender، EmailSender، SmsSender و ReservationService را تغییر دهیم (۴ تغییر). در معماری اصلاح‌شده فقط افزودن یک کلاس SmsSender و ثبت آن در NotificationDispatcher کافی است، بنابراین با ۲ تغییر قابلیت اضافه می‌شود.
- **پرداخت حضوری:** قبل از بازطراحی، PaymentProcessor و ReservationService هر دو تغییر می‌کردند (۲ تغییر). اکنون صرفا پیاده‌سازی جدیدی مانند OnSitePaymentHandler ایجاد می‌شود و در نگاشت PaymentProcessor ثبت می‌گردد. در نتیجه ۲ تغییر به ۲ افزودن متمرکز و بدون دست‌کاری سرویس رزرو کاهش یافته است.

## گام پنجم - نتیجه‌گیری

با رعایت اصول شی‌گرایی (به‌ویژه SRP، OCP، DIP و ISP) مسیر تغییرات کوتاه و قابل پیش‌بینی می‌شود. هر ماژول تنها یک دلیل برای تغییر دارد، وابستگی‌ها به اینترفیس‌ها تزریق می‌شوند و افزودن قابلیت‌های جدید بدون ویرایش کلاس‌های موجود انجام می‌شود. این ویژگی‌ها موجب نگهداشت‌پذیری بالاتر و امکان تست واحد مستقل در توسعه‌های بعدی می‌شود زیرا تیم توسعه می‌تواند تنها روی ماژول مرتبط با قابلیت جدید کار کرده و سایر بخش‌ها را دست‌نخورده باقی بگذارد.
